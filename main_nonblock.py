...
parser.add_argument('--model-path', type=str, default='aim_csgo/models/yolov5s6.pt', help='模型地址')
parser.add_argument('--imgsz', type=int, default=1280, help='和你训练模型时imgsz一样')
...
parser.add_argument('--thickness', type=int, default=4, help='画框粗细，必须大于1/resize-window')
...
parser.add_argument('--region', type=tuple, default=(0.5, 0.4),
...
parser.add_argument('--lock-sen', type=float, default=1.5,
                    help='lock幅度系数；若在桌面试用请调成1，在游戏中(csgo)则为灵敏度')
parser.add_argument('--lock-smooth', type=float, default=1.5, help='lock平滑系数；越大越平滑，最低1.0')
parser.add_argument('--lock-button', type=str, default='right', help='lock按键；只支持鼠标按键')
...
parser.add_argument('--recoil-button-ak47', type=str, default='x2',
                    help='ak47压枪按键；压枪时不会lock，只支持鼠标按键,用不到置为0')
